...
public interface BidRepository extends JpaRepository<Bid, Long> {
    ...
    @Query(value = "SELECT bidder_name AS bidderName, bid_date AS bidDate FROM bid GROUP BY bidder_id WHERE lot_id = ?,bidder_id = ?  ORDER BY bidder_date ASC LIMIT 1", nativeQuery = true)
    Long findByBidDateMin(int bidderId, Long lotId);
    @Query(value = "SELECT bidder_name AS bidderName, bid_date AS bidDate FROM bid GROUP BY bidder_id WHERE lot_id = ?,bidder_id = ?  ORDER BY bidder_date DESC LIMIT 1", nativeQuery = true)
